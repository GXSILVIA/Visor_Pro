                ...
                if modo == "Crecimiento":
                    ...
                    st.markdown(f"""
                        <div style="display: flex; justify-content: space-around; background: #1e1e1e; padding: 20px; border-radius: 12px; border: 1px solid #444; text-align: center;">
                            ...
                            <div style="border-left: 1px solid #444; padding-left: 20px;"><p style="color: #28a745; font-weight: bold; margin:0;">🟢 Bajo</p><h2 style="margin:0; color: #28a745;">{round(b/t_e*100,1)}%</h2><p style="color:#28a745; margin:0;">VRs: {b}</p><p style="color:#28a745; margin:0;">V. Prom: {round(df_ex[df_ex['Traslape']<=25]['VOL'].mean() if b>0 else 0,1)}</p></div>
                            <div><p style="color: #ffc107; font-weight: bold; margin:0;">🟡 Medio</p><h2 style="margin:0; color: #ffc107;">{round(m_v/t_e*100,1)}%</h2><p style="color:#ffc107; margin:0;">VRs: {m_v}</p><p style="color:#ffc107; margin:0;">V. Prom: {round(df_ex[(df_ex['Traslape']>25) & (df_ex['Traslape']<=50)]['VOL'].mean() if m_v>0 else 0,1)}</p></div>
                            <div><p style="color: #fd7e14; font-weight: bold; margin:0;">🟠 Alto</p><h2 style="margin:0; color: #fd7e14;">{round(a/t_e*100,1)}%</h2><p style="color:#fd7e14; margin:0;">VRs: {a}</p><p style="color:#fd7e14; margin:0;">V. Prom: {round(df_ex[(df_ex['Traslape']>50) & (df_ex['Traslape']<=75)]['VOL'].mean() if a>0 else 0,1)}</p></div>
                            <div><p style="color: #dc3545; font-weight: bold; margin:0;">🔴 Crítico</p><h2 style="margin:0; color: #dc3545;">{round(c/t_e*100,1)}%</h2><p style="color:#dc3545; margin:0;">VRs: {c}</p><p style="color:#dc3545; margin:0;">V. Prom: {round(df_ex[df_ex['Traslape']>75]['VOL'].mean() if c>0 else 0,1)}</p></div>
